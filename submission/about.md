...
Somewhere between OpenAI, Anthropic, Gemini, a gateway, and models running on
my own machine, deciding *which model handles which job* had quietly become a
small accounting practice. Prices per million tokens in one place, a benchmark
report in another, and the part nobody writes down: which providers I am
contractually not allowed to send customer data to.
...
- **Eight of them never returned a single successful call** — and my routing
  config was still pointing at one of them.

I had the numbers and still could not act on them, because the decision has
four dimensions that do not reduce to one: money, latency tail, reliability, and
data governance. That is the kind of problem an agent should be good at. But I
was not about to hand an agent the ability to redirect my spend on the strength
of a screenshot.

WebMCP is what closes that gap. Not because an agent can click my buttons —
because I get to decide exactly which operations exist.
...
The agent sees fifteen tools on `document.modelContext`. Ten are read-only: the
priced catalogue joined with my own measurements, the traffic classes and their
constraints, `find_waste`, `compare_models`, and `simulate_policy`, which
projects what a candidate routing policy would cost and deliver without applying
anything.
...
The interesting operations here are not clicks. There is no button for
"project my monthly cost under this arbitrary rule set," and there could not be.
That is why this belongs in a tool surface rather than a DOM.
...
accept is not the agent's call. Once I accept thirty-day retention, it can
route the class. That handoff is the whole project.
...
field allowlist and a secret scan — prices and governance terms from published
provider docs, latency and success rates from the measurements. What is *not*
measured (call volumes, budget caps, class constraints) is operator input,
editable, and labelled as such everywhere it appears. `get_provenance` exists so
an agent can state the limits of its evidence before recommending anything,
including a warning that per-model sample sizes are small and p95 is coarse.

Three coding agents worked on it, and the division mattered more than I
expected — each reached somewhere the others could not.
...
measurements slipped past quality gates, latency ceilings and success floors
with no blocker **and no warning**. I had fixed the exact same mistake in the
...
So after a rejection the agent does not know until it polls
`get_proposal_status`, which is why the video shows me telling it to read my
note. That is a limit of the standard, and worth knowing if you are designing
around it.
...
bytes. The re-take used ffmpeg, verified against a real frame *before* recording.
...
were invisible from anywhere else. Gemini audited the repository cold, with no
context from either, and caught five stale numbers in my own documentation plus
the hole where unmeasured models passed every performance constraint. It also
tried to falsify the approval guard by tracing call paths, and confirmed it
holds.
...
ranking formula, and admitting that p95 from thirty-three samples is coarse. The
agent then cited that caveat back at me unprompted while justifying a 187 ms
margin it refused to call compliant.

## What's next

The notification gap is the interesting thread. A page can fire `toolchange`,
so the tool list itself could carry state — a decision could surface as a tool
appearing rather than something the agent has to poll for. Whether clients act
on that is unknown, and worth finding out.

Beyond that: real billing APIs instead of declared volumes, and per-model
sample sizes large enough that a p95 difference of 187 ms would mean something.
